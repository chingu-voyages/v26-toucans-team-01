...
const principleInterestInputElem = document.getElementById(
...
var downPaymentAmountInput = Number(
  formatCurrencyIn(downPaymentAmountInputElem.value)
);
var downPaymentPercentInput = Number(
  formatPercentageIn(downPaymentPercentInputElem.value)
);
...
var principleInterestInput = Number(
  formatCurrencyIn(principleInterestInputElem.value)
);
...
function updateInput() {
  ...
  downPaymentAmountInput = Number(
    formatCurrencyIn(downPaymentAmountInputElem.value)
  );
  downPaymentPercentInput = Number(
    formatPercentageIn(downPaymentPercentInputElem.value)
  );
...
function updateOutput() {
  ...
  downPaymentPercentInputElem.value = formatPercentageOut(
...
function calculateDownPayPercent(homePrice, downPaymentAmount) {
  ...
  homePrice.toString().replace(/\$|\,/g, '');
  if (homePrice > 0) {
    downPaymentPercentInput = parseFloat(
      (downPaymentAmount / homePrice) * 100
    ).toFixed(3);
  } else {
    downPaymentPercentInput = 0.0;
  }
  downPaymentPercentInputElem.value = formatPercentageOut(
    downPaymentPercentInput
  );
...
function calculateDownPayAmount(homePrice, downPaymentPercent) {
  downPaymentAmountInput = parseFloat(
 ...
 */
function calculatePrincipleAndInterest(
  calcPrinciple,
  calcLoanLength,
  calcInterest
) {
  if (calcInterest > 0 && calcLoanLength > 0) {
    principleInterestInput = parseFloat(
      (calcPrinciple *
        calcInterest *
        Math.pow(1 + parseFloat(calcInterest), calcLoanLength)) /
        (Math.pow(1 + parseFloat(calcInterest), calcLoanLength) - 1)
    ).toFixed(2);
  } else if (calcLoanLength > 0) {
    principleInterestInput = parseFloat(calcPrinciple / calcLoanLength).toFixed(
      2
    );
  } else {
...
function calculateHomePrice(
  ...
  //console.log(homePriceInput +"is type"+typeof(homePrinceInput));
  homePriceInputElem.value = formatCurrencyOut(homePriceInput);
  homePriceSliderInputElem.value = homePriceInput;
 ...
 */
function deriveLoanVariablesAfterUpdate(
  homePrice,
  downPaymentAmount,
  interestRate,
  loanLength
...
homePriceInputElem.addEventListener('input', function () {
  ...
  deriveLoanVariablesAfterUpdate(
    updatedHomePrice,
